...
for yourself.  wakterm should run on any modern unix as well as Windows 10 and
macOS.
...
* Run it via either `cargo run --release --bin wakterm` or `target/release/wakterm`

You will need a collection of support libraries; the [`get-deps`](https://github.com/wakamex/wakterm/blob/main/get-deps) script will
attempt to install them for you.  If it doesn't know about your system,
[please contribute instructions!](https://github.com/wakamex/wakterm/blob/main/CONTRIBUTING.md).

If you don't plan to submit a pull request to the wakterm repo, you can
...
$ ./get-deps
$ cargo build --release
$ cargo run --release --bin wakterm -- start
```
...
```console
$ curl https://sh.rustup.rs -sSf | sh -s
$ git clone --depth=1 --branch=main --recursive https://github.com/wakamex/wakterm.git
$ cd wakterm
...
If your distribution has X11 but not Wayland, then you can build wakterm without
...
only supported way to build wakterm.
