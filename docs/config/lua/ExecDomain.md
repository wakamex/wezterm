...
wakterm to explicitly spawn a command that runs `docker exec`, you would also
...
You must use the `wakterm.exec_domain` function to define a domain. It accepts
the following parameters:

```
wakterm.exec_domain(NAME, FIXUP [, LABEL])
```
...
```lua
wakterm.exec_domain('myname', function(cmd)
  return cmd
end)
```
...
what wakterm will execute in order to satisfy the user's request to
spawn a new program.
...
-- wakterm.exec_domain()
wakterm.exec_domains(domain_name, fixup_func, function(domain_name)
  return domain_name
end)
```
...
[wakterm.format()](wakterm/format.md) to manage that.
...
config.exec_domains = {
  ...
  -- killed.
  wakterm.exec_domain('scoped', function(cmd)
    -- The "cmd" parameter is a SpawnCommand object.
    -- You can log it to see what's inside:
    wakterm.log_info(cmd)

    -- Synthesize a human understandable scope name that is
    -- (reasonably) unique. WAKTERM_PANE is the pane id that
    -- will be used for the newly spawned pane.
    -- WAKTERM_UNIX_SOCKET is associated with the wakterm
    -- process id.
    local env = cmd.set_environment_variables
    local ident = 'wakterm-pane-'
      .. env.WAKTERM_PANE
      .. '-on-'
      .. basename(env.WAKTERM_UNIX_SOCKET)
    ...
    local wrapped = {
      ...
      '--user',
      '--scope',
      '--description=Shell started by wakterm',
      '--same-dir',
      '--collect',
      '--unit=' .. ident,
    }
...
-- spawned by wakterm will have its own scope
...
function docker_list()
  local docker_list = {}
  local success, stdout, stderr = wakterm.run_child_process {
    'docker',
    'container',
    'ls',
    '--format',
  ...
  for _, line in ipairs(wakterm.split_by_newlines(stdout)) do
