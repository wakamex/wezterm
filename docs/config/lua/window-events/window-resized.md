...
live resize operation.  `wakterm` will coalesce the stream of multiple events
...
wakterm.on('window-resized', function(window, pane)
  recompute_padding(window)
end)

wakterm.on('window-config-reloaded', function(window)
  recompute_padding(window)
end)
```
