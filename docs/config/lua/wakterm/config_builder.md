...
local config = {}
if wakterm.config_builder then
  config = wakterm.config_builder()
end
...
When evaluated by earlier versions of wakterm, this config will produce the
...
```
11:44:11.668  WARN   wakterm_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
```

When using the config builder, the warning message is improved:

```
11:45:23.774  WARN   wakterm_dynamic::error > `wrong` is not a valid Config field.  There are too many alternatives to list here; consult the documentation!
...
The consequence of an error is that wakterm will show a configuration error
