...
is emitted.  Events can be emitted by wakterm itself, or through code/configuration
that you specify.

`wakterm.on` can register multiple callbacks for the same event; internally
...
See [wakterm.action_callback](./action_callback.md) for a helper to define a custom action callback.
...
You may register handlers for arbitrary events for which wakterm itself
has no special knowledge.  It is recommended that you avoid event names
that are likely to be used future versions of wakterm in order to avoid
unexpected behavior if/when those names might be used in future.

The [wakterm.emit](emit.md) function and the [EmitEvent](../keyassignment/EmitEvent.md) key assignment can be used to
emit events.
...
wakterm.on('trigger-vim-with-scrollback', function(window, pane)
