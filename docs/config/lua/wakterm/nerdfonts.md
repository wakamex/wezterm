...
status lines and titles in your wakterm configuration.

wakterm includes `Nerd Font Symbols Font` as a default font fallback
which means that these special symbols are available even without 
requiring you to use a patched font.

This example shows how to lookup `wakterm.nerdfonts.fa_clock_o`, the Font
...
wakterm.on('update-right-status', function(window, pane)
  ...
  window:set_right_status(wakterm.format {
    { Text = wakterm.nerdfonts.fa_clock_o .. ' ' .. date },
  })
end)
```
