---
title: wakterm.font_with_fallback
tags:
 - font
---

# `wakterm.font_with_fallback(families [, attributes])`
...
return {
  font = wakterm.font_with_fallback { 'JetBrains Mono', 'Noto Color Emoji' },
}
```

wakterm implicitly adds its default fallback to the list that you specify.

The *attributes* parameter behaves the same as that of [wakterm.font](font.md)
...
= `true` will cause wakterm to try to automatically scale using the
