---
title: wakterm.get_builtin_color_schemes
tags:
 - color
 - scheme
 - theme
...
couple of entries just from your `wakterm.lua` configuration file.

This example shows how to make wakterm pick a random color scheme for
...
local schemes = {}
for name, scheme in pairs(wakterm.get_builtin_color_schemes()) do
...
local scheme = wakterm.get_builtin_color_schemes()['Gruvbox Light']
...
local function dark_schemes()
  local schemes = wakterm.get_builtin_color_schemes()
  local dark = {}
  for name, scheme in pairs(schemes) do
    -- parse into a color object
    local bg = wakterm.color.parse(scheme.background)
...
but can still be called as `wakterm.get_builtin_color_schemes()`. See that page
for more examples.
