...
# `wakterm.font(family [, attributes])`
...
return {
  font = wakterm.font 'JetBrains Mono',
}
```
...
oblique) for non-bitmap fonts, wakterm can only select and use fonts that you
...
return {
  font = wakterm.font('JetBrains Mono', { weight = 'Bold' }),
}
```

When resolving fonts from [font_dirs](../config/font_dirs.md), wakterm follows CSS Fonts
...
[wakterm.font_with_fallback](font_with_fallback.md) when you want to specify precise
