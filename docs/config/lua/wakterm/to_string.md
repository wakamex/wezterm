...
to remain the same across different versions of wakterm.
...
conversions of the parameters passed to [wakterm.log_info](log_info.md).

```lua
local wakterm = require 'wakterm'
assert(wakterm.to_string { 1, 2 } == [=[[
    1,
    2,
]]=])
assert(wakterm.to_string { a = 1, b = 2 } == [[{
    "a": 1,
    "b": 2,
}]])
```
