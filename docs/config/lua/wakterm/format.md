...
`wakterm.format` can be used to produce a formatted string
with terminal graphic attributes such as bold, italic and colors.
The resultant string is rendered into a string with wakterm
compatible escape sequences embedded.

`wakterm.format` accepts a single array argument, where each
element is a `FormatItem`.

This example logs the text `Hello`, then the date/time, underlined, in purple
text on a blue background to the stderr of the wakterm process:

```lua
local wakterm = require 'wakterm'

local success, date, stderr = wakterm.run_child_process { 'date' }

wakterm.log_info(wakterm.format {
