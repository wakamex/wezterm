...
* `message` - the text to show for confirmation. You may embed
  escape sequences and/or use [wakterm.format](../wakterm/format.md).
  Defaults to: `"🛑 Really continue?"`.
* `action` - event callback registered via `wakterm.action_callback`.  The
  ...
  user selects `Yes`.
* `cancel` - event callback registered via `wakterm.action_callback`.  The
  callback's function signature is `(window, pane)` where `window` and
  `pane` are the [Window](../window/index.md) and [Pane](../pane/index.md).
