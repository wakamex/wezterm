...
the default domain to instead use a particular WSL distribution so that wakterm
...
that wakterm can then use [shell integration](../../../shell-integration.md) to
...
then wakterm will by default create a `WslDomain` with the name `"WSL:Ubuntu-18.04"`
...
then when wakterm starts up, it will open with a shell running inside that Ubuntu
