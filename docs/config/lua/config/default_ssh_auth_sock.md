...
The following wakterm configuration snippet shows how to detect when gnome
...
then
  local onep_auth =
    string.format('%s/.1password/agent.sock', wakterm.home_dir)
  ...
  -- would be 0
  if #wakterm.glob(onep_auth) == 1 then
