...
By default, wakterm will use an appropriate system-specific method for
locating the fonts that you specify using the options below.  In addition,
if you configure the `font_dirs` option, wakterm will load fonts from that
set of directories:

```lua
-- This tells wakterm to look first for fonts in the directory named
-- `fonts` that is found alongside your `wakterm.lua` file.
-- As this option is an array, you may list multiple locations if
-- you wish.
config.font_dirs = { 'fonts' }
```

wakterm will scan the `font_dirs` to build a database of available fonts.  When
resolving a font, wakterm will first use the configured
