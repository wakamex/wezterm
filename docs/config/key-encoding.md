...
The default encoding scheme used by wakterm is xterm compatible, but there are
...
wakterm will honor the application requests to modify the keyboard encoding.

## Windows

On Windows, [allow_win32_input_mode](lua/config/allow_win32_input_mode.md)
defaults to `true` which causes wakterm to listen for an escape sequence
