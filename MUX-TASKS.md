...
- [x] H1 confirmed and fixed: PDU allocation unbounded → added MAX_PDU_SIZE (64 MiB) check
- [x] H3 investigated: redraw storm (#7540) matches our resize fix symptoms exactly (focus flapping + pane size divergence). Likely fixed by `reconcile_tree_sizes` + `ResizeTab` + focus loop fix.
- [x] H4 confirmed: `rotate_clockwise`/`rotate_counter_clockwise` has no PDU — server tree diverges. Needs protocol addition (new PDU type). Deferred.
