...
fn derive_struct(input: &DeriveInput, fields: &FieldsNamed) -> Result<TokenStream> {
    ...
    let bound = parse_quote!(wakterm_dynamic::FromDynamic);
    ...
    let from_dynamic = match info.try_from {
        Some(try_from) => {
            quote!(
                ...
                <#ident>::try_from(target).map_err(|e| wakterm_dynamic::Error::Message(format!("{:#}", e)))
            )
        }
        None => {
            quote!(
                match value {
                    Value::Object(obj) => {
                        wakterm_dynamic::Error::raise_unknown_fields(options, #literal, &obj, Self::possible_field_names())?;
                        #obj
                    }
    ...
    let tokens = quote! {
        impl #impl_generics wakterm_dynamic::FromDynamic for #ident #ty_generics #bounded_where_clause {
            fn from_dynamic(value: &wakterm_dynamic::Value, options: wakterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, wakterm_dynamic::Error> {
                use wakterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #adjust_options
                #from_dynamic
            }
...
fn derive_enum(input: &DeriveInput, enumeration: &DataEnum) -> Result<TokenStream> {
                        ...
                        quote!(
                            #literal => {
                                match value {
                                    ...
                                    other => return Err(wakterm_dynamic::Error::NoConversion {
                                        source_type: other.variant_name().to_string(),
                                        dest_type: "Object",
                                    }),
                                }
                            ...
                            let var_fields = fields
                                ...
                                .map(|(idx, f)| {
                                    ...
                                    quote!(
                                        <#ty>::from_dynamic(
                                            arr.get(#idx)
                                            .ok_or_else(|| wakterm_dynamic::Error::Message(
                    ...
                    match value {
                        ...
                        Value::Object(place) => {
                            if place.len() == 1 {
                                ...
                                Err(wakterm_dynamic::Error::IncorrectNumberOfEnumKeys {
                                    type_name: #literal,
                                    num_keys: place.len(),
                                })
                            }
    ...
    let tokens = quote! {
        impl wakterm_dynamic::FromDynamic for #ident {
            fn from_dynamic(value: &wakterm_dynamic::Value, options: wakterm_dynamic::FromDynamicOptions) -> core::result::Result<Self, wakterm_dynamic::Error> {
                use wakterm_dynamic::{Value, BorrowedKey, ObjectKeyTrait};
                #from_dynamic
            }
