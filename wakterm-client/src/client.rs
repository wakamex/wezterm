...
fn process_unilateral(
    ...
    match &decoded.pdu {
        ...
        Pdu::TabResized(_) => {
            // Resync to pick up size changes from other clients.
            // The server suppresses self-echo TabResized notifications,
            // so this only fires for resizes from OTHER sessions.
            log::trace!("resync due to {:?}", decoded.pdu);
            promise::spawn::spawn_into_main_thread(async move {
                let mux = Mux::try_get().ok_or_else(|| anyhow!("no more mux"))?;
                let client_domain = mux
...
impl Reconnectable {
    ...
    /// Resolve the path to wakterm for the remote system.
    ...
    /// `path` comes from the SshDoman::remote_wakterm_path option; if set
    ...
    /// message to the user.
    fn wakterm_bin_path(path: &Option<String>) -> String {
        path.as_deref().unwrap_or("wakterm").to_string()
    }
    ...
    fn ssh_connect(
        ...
        let proxy_bin = Self::wakterm_bin_path(&ssh_dom.remote_wakterm_path);
        ...
        if let Some(Ok(ssh_params)) = tls_client.ssh_parameters() {
            if self.tls_creds.is_none() {
                ...
                let creds = ui.run_and_log_error(|| {
                    ...
                    let cmd = format!(
                        "{} cli tlscreds",
                        Self::wakterm_bin_path(&tls_client.remote_wakterm_path)
                    );
...
impl Client {
    ...
    fn compute_unix_domain(
        ...
        match std::env::var_os("WAKTERM_UNIX_SOCKET") {
            ...
            Some(_) | None => {
                ...
                Ok(config
                    ...
                    .ok_or_else(|| {
                        anyhow!(
                            "no default unix domain is configured and WAKTERM_UNIX_SOCKET \
    ...
    pub async fn resolve_pane_id(&self, pane_id: Option<PaneId>) -> anyhow::Result<PaneId> {
        let pane_id: PaneId = match pane_id {
            Some(p) => p,
            None => {
                if let Ok(pane) = std::env::var("WAKTERM_PANE") {
                    pane.parse()?
                } else {
                    ...
                    if clients.is_empty() {
                        anyhow::bail!(
                            "--pane-id was not specified and $WAKTERM_PANE
