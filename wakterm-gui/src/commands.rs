...
impl CommandDef {
    ...
    pub fn recreate_menubar(config: &ConfigHandle) {
        ...
        let wakterm_perform_key_assignment_sel = sel!(waktermPerformKeyAssignment:);
        ...
        let main_menu = match Menu::get_main_menu() {
            Some(existing) => {
                mark_candidates(
                    ...
                    wakterm_perform_key_assignment_sel,
                );
        ...
        let mut order: Vec<&'static str> = vec!["wakterm", "Shell", "Edit", "View", "Window"];
        ...
        for &title in &order {
            for cmd in &commands {
                ...
                let mut submenu = main_menu.get_or_create_sub_menu(&cmd.menubar[0], |menu| {
                    if cmd.menubar[0] == "Window" {
                    ...
                    } else if cmd.menubar[0] == "wakterm" {
                        menu.assign_as_app_menu();

                        let about_item = MenuItem::new_with(
                            &format!("wakterm {}", config::wakterm_version()),
                            Some(wakterm_perform_key_assignment_sel),
                            "",
                        );
                        ...
                        about_item.set_represented_item(RepresentedItem::KeyAssignment(
                            KeyAssignment::CopyTextTo {
                                text: config::wakterm_version().to_string(),
...
pub fn derive_command_from_key_assignment(action: &KeyAssignment) -> Option<CommandDef> {
    Some(match action {
        ...
        QuitApplication => CommandDef {
            brief: "Quit wakterm".into(),
            doc: "Quits wakterm".into(),
        ...
        OpenUri(uri) => match uri.as_ref() {
            "https://wakterm.org/" => CommandDef {
                brief: "Documentation".into(),
                doc: "Visit the wakterm documentation website".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("md_help"),
            },
            "https://github.com/wakamex/wakterm/discussions/" => CommandDef {
                brief: "Discuss on GitHub".into(),
                doc: "Visit wakterm's GitHub discussion".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("oct_comment_discussion"),
            },
            "https://github.com/wakamex/wakterm/issues/" => CommandDef {
                brief: "Search or report issue on GitHub".into(),
                doc: "Visit wakterm's GitHub issues".into(),
                keys: vec![],
                args: &[],
                menubar: &["Help"],
                icon: Some("fa_ticket"),
...
fn compute_default_actions() -> Vec<KeyAssignment> {
    ...
    return vec![
        ...
        OpenUri("https://github.com/wakamex/wakterm/issues/".to_string()),
        ShowDebugOverlay,
        // ----------------- Misc
