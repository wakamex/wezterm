use crate::ICON_DATA;
use anyhow::anyhow;
use config::{configuration, wakterm_version};
...
fn get_github_release_info(uri: &str) -> anyhow::Result<Release> {
    ...
    let _res = Request::new(&uri)
        ...
        .header(
            "User-Agent",
            &format!("wakterm/wakterm-{}", wakterm_version()),
        )
        .send(&mut latest)
...
pub fn get_latest_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/wakamex/wakterm/releases/latest")
}

#[allow(unused)]
pub fn get_nightly_release_info() -> anyhow::Result<Release> {
    get_github_release_info("https://api.github.com/repos/wakamex/wakterm/releases/tags/nightly")
}
...
pub fn load_last_release_info_and_set_banner() {
    ...
    if let Ok(data) = std::fs::read(update_file_name) {
        ...
        let current = wakterm_version();
        let force_ui = std::env::var_os("WAKTERM_ALWAYS_SHOW_UPDATE_UI").is_some();
...
fn set_banner_from_release_info(latest: &Release) {
    ...
    let url = format!("https://wakterm.org/changelog.html#{}", latest.tag_name);
...
fn update_checker() {
    ...
    loop {
        // Figure out which other wakterm-guis are running.
        ...
        let socks = wakterm_client::discovery::discover_gui_socks();
