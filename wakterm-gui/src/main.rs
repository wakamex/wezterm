...
use wakterm_bidi::Direction;
use wakterm_client::domain::ClientDomain;
use wakterm_font::shaper::PresentationWidth;
use wakterm_font::FontConfiguration;
use wakterm_gui_subcommands::*;
use wakterm_mux_server_impl::update_mux_domains;
use wakterm_toast_notification::*;
...
enum SubCommand {
    ...
    #[command(name = "connect", about = "Connect to wakterm multiplexer")]
...
async fn async_run_ssh(opts: SshCommand) -> anyhow::Result<()> {
    ...
    if opts.verbose {
        ssh_option.insert("wakterm_ssh_verbose".to_string(), "true".to_string());
    }
...
async fn async_run_terminal_gui(
    ...
    std::env::set_var("WAKTERM_UNIX_SOCKET", unix_socket_path.clone());
...
impl Publish {
    pub fn resolve(mux: &Arc<Mux>, config: &ConfigHandle, always_new_process: bool) -> Self {
        ...
        match wakterm_client::discovery::resolve_gui_sock_path(
    ...
    pub fn try_spawn(
        ...
        if let Publish::TryPathOrPublish(gui_sock) = &self {
            ...
            match wakterm_client::client::Client::new_unix_domain(None, &dom, false, &mut ui, true)
            {
                Ok(client) => {
                    ...
                    let res = block_on(executor.run(async move {
                        ...
                        if vers.config_file_path
                            != std::env::var_os("WAKTERM_CONFIG_FILE").map(Into::into)
                        {
                            *self = Publish::NoConnectNoPublish;
                            anyhow::bail!(
                    ...
                    match res {
                        Ok(res) => {
                            log::info!(
                             ...
                             Use wakterm start --always-new-process if you do not want this behavior. \
                             Result={:?}",
                                res
                            );
                            Ok(true)
...
fn spawn_mux_server(unix_socket_path: PathBuf, should_publish: bool) -> anyhow::Result<()> {
    let mut listener =
        wakterm_mux_server_impl::local::LocalListener::with_domain(&config::UnixDomain {
    ...
    std::thread::spawn(move || {
        let name_holder;
        if should_publish {
            name_holder = wakterm_client::discovery::publish_gui_sock_path(
                &unix_socket_path,
                &crate::termwindow::get_window_class(),
            );
...
fn run_terminal_gui(opts: StartCommand, default_domain_name: Option<String>) -> anyhow::Result<()> {
    ...
    // First, let's see if we can ask an already running wakterm to do this.
...
pub fn run_ls_fonts(config: config::ConfigHandle, cmd: &LsFontsCommand) -> anyhow::Result<()> {
    use wakterm_font::parser::ParsedFont;
    ...
    let font_config = Rc::new(wakterm_font::FontConfiguration::new(
        Some(config.clone()),
    ...
    if let Some(text) = &text {
        ...
        let ft_lib = wakterm_font::ftwrap::Library::new()?;
            ...
            while let Some(info) = iter.next() {
                ...
                if config.custom_block_glyphs {
                    if let Some(block) = info.only_char.and_then(BlockKey::from_char) {
                        ...
                        println!(
                            "{:2} {:4} {:12} drawn by wakterm because custom_block_glyphs=true: {:?}",
...
fn run() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    {
        unsafe {
            ::windows::Win32::UI::Shell::SetCurrentProcessExplicitAppUserModelID(
                ::windows::core::PCWSTR(wide_string("org.wezfurlong.wakterm").as_ptr()),
            )
            .unwrap();
        }
    }
    ...
    let sub = match opts.cmd.as_ref().cloned() {
        ...
        None => {
            ...
            let mut argv = vec!["wakterm-gui".to_string()];
