...
use wakterm_font::units::{IntPixelLength, PixelLength};
use wakterm_font::{ClearShapeCache, GlyphInfo, LoadedFont};
use wakterm_term::color::{ColorAttribute, ColorPalette};
use wakterm_term::{CellAttributes, Line, StableRowIndex};
...
fn resolve_fg_color_attr(
    ...
    match fg {
        wakterm_term::color::ColorAttribute::Default => {
        ...
        wakterm_term::color::ColorAttribute::PaletteIndex(idx)
            if idx < 8 && config.bold_brightens_ansi_colors != BoldBrightening::No =>
        {
            // For compatibility purposes, switch to a brighter version
            // of one of the standard ANSI colors when Bold is enabled.
            // This lifts black to dark grey.
            let idx = if attrs.intensity() == wakterm_term::Intensity::Bold {
                idx + 8
            } else {
                idx
            };

            palette.resolve_fg(wakterm_term::color::ColorAttribute::PaletteIndex(idx))
        }
