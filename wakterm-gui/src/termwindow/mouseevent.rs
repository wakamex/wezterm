...
use wakterm_dynamic::ToDynamic;
use wakterm_term::input::{MouseButton, MouseEventKind as TMEK};
use wakterm_term::{ClickPosition, LastMouseClick, StableRowIndex};
...
impl super::TermWindow {
    ...
    fn mouse_event_terminal(
                    ...
                    match &event.kind {
                        ...
                        WMEK::Move => {
                            if self.config.pane_focus_follows_mouse {
                                let mux = Mux::get();
                                if let Some(tab) = mux
                                    .get_active_tab_for_window_for_current_identity(
                                        self.mux_window_id,
                                    )
                                {
                                    self.activate_local_pane_index(&tab, pos.index);
        ...
        let mouse_event = wakterm_term::MouseEvent {
