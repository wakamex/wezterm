...
use ::wakterm_term::input::{ClickPosition, MouseButton as TMB};
use ::window::*;
...
use wakterm_term::input::LastMouseClick;
use wakterm_term::{Alert, Progress, StableRowIndex, TerminalConfiguration, TerminalSize};
...
pub enum TermWindowNotif {
    ...
    GetConfigOverrides(Sender<wakterm_dynamic::Value>),
    SetConfigOverrides(wakterm_dynamic::Value),
...
impl TermWindow {
    ...
    fn created(&mut self, ctx: RenderContext) -> anyhow::Result<()> {
        ...
        match RenderState::new(ctx, &self.fonts, &self.render_metrics, ATLAS_SIZE) {
            Ok(render_state) => {
                log::debug!(
                    "OpenGL initialized! {} wakterm version: {}",
                    render_info,
    ...
    fn dispatch_window_event(
        ...
        match event {
            WindowEvent::Destroyed => {
                ...
                // <https://github.com/wakamex/wakterm/issues/3522>
            ...
            WindowEvent::AppearanceChanged(appearance) => {
                ...
                // <https://github.com/wakamex/wakterm/issues/2295>
                config::reload();
        ...
        match notif {
            ...
            TermWindowNotif::PerformAssignment {
                ...
                let result = || -> anyhow::Result<()> {
                    ...
                    // <https://github.com/wakamex/wakterm/issues/3209>
...
impl TermWindow {
    ...
    fn get_semantic_prompt_zones(&mut self, pane: &Arc<dyn Pane>) -> &[StableRowIndex] {
        ...
        if cache.seqno != seqno {
            ...
            let mut zones: Vec<StableRowIndex> = zones
                .into_iter()
                .filter_map(|zone| {
                    if zone.semantic_type == wakterm_term::SemanticType::Prompt {
                        Some(zone.start_y)
                    } else {
                        None
                    }
            ...
            // <https://github.com/wakamex/wakterm/issues/1121>
            zones.dedup();
            cache.zones = zones;
            cache.seqno = seqno;
        }
    ...
    pub fn perform_key_assignment(
        ...
        match assignment {
            ...
            OpenUri(link) => {
                wakterm_open_url::open_url(link);
            }
