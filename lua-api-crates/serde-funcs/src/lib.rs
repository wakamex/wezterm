...
use wakterm_dynamic::{FromDynamic, Value as DynValue};
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    wakterm_mod.set("json_parse", lua.create_function(json_decode)?)?;
    wakterm_mod.set("json_encode", lua.create_function(json_encode)?)?;
            ...
            Ok(mt) => {
                if let Ok(to_dynamic) = mt.get::<mlua::Function>("__wakterm_to_dynamic") {
                    ...
                    return Err(mlua::Error::FromLuaConversionError {
                        from: "userdata",
                        to: "wakterm_dynamic::Value",
                        message: Some(format!("no __wakterm_to_dynamic metadata")),
                    });
                }
            }
            Err(err) => {
