...
# This file hooks up shell integration for wakterm.
# It is suitable for zsh and bash.
#
# Although wakterm is mentioned here, the sequences used are not wakterm
...
#
# WAKTERM_SHELL_SKIP_ALL - disables all
# WAKTERM_SHELL_SKIP_SEMANTIC_ZONES - disables zones
...
if [ "${WAKTERM_SHELL_SKIP_ALL-}" = "1" ] ; then
  return 0
fi
...
} # end of __wakterm_install_bash_prexec
...
# command provided by wakterm if wakterm is installed, but falls
# back to a simple printf command otherwise.
__wakterm_osc7() {
  if hash wakterm 2>/dev/null ; then
    wakterm set-working-directory 2>/dev/null && return 0
    # If the command failed (perhaps the installed wakterm
...
__wakterm_semantic_precmd_executing=""
__wakterm_semantic_precmd() {
  local ret="$?"
  if [[ "$__wakterm_semantic_precmd_executing" != "0" ]] ; then
    __wakterm_save_ps1="$PS1"
    __wakterm_save_ps2="$PS2"
    ...
    __wakterm_check_ps1="$PS1"
  fi
  if [[ "$__wakterm_semantic_precmd_executing" != "" ]] ; then
  ...
  __wakterm_semantic_precmd_executing=0
}

function __wakterm_semantic_preexec() {
  # Restore the original PS1/PS2 if set
  if [[ -n "${__wakterm_save_ps1+1}" && "${__wakterm_check_ps1-}" == "${PS1}" ]]; then
	  PS1="$__wakterm_save_ps1"
	  PS2="$__wakterm_save_ps2"
	  unset __wakterm_save_ps1
  fi
  # Indicate that the command output begins here
  printf "\033]133;C;\007"
  __wakterm_semantic_precmd_executing=1
}

__wakterm_user_vars_precmd() {
  __wakterm_set_user_var "WAKTERM_PROG" ""
  __wakterm_set_user_var "WAKTERM_USER" "$(id -un)"

  # Indicate whether this pane is running inside tmux or not
  if [[ -n "${TMUX-}" ]] ; then
    __wakterm_set_user_var "WAKTERM_IN_TMUX" "1"
  else
    __wakterm_set_user_var "WAKTERM_IN_TMUX" "0"
  fi

  # You may set WAKTERM_HOSTNAME to a name you want to use instead
  # of calling out to the hostname executable on every prompt print.
if [[ -z "${WAKTERM_HOSTNAME}" ]]; then
  if [[ -r /proc/sys/kernel/hostname ]]; then
    __wakterm_set_user_var "WAKTERM_HOST" "$(cat /proc/sys/kernel/hostname)"
  elif hash hostname 2>/dev/null; then
    __wakterm_set_user_var "WAKTERM_HOST" "$(hostname)"
  elif hash hostnamectl 2>/dev/null; then
    __wakterm_set_user_var "WAKTERM_HOST" "$(hostnamectl hostname)"
  else
    __wakterm_set_user_var "WAKTERM_HOST" "unknown"
  fi
else
  __wakterm_set_user_var "WAKTERM_HOST" "${WAKTERM_HOSTNAME}"
fi

}

__wakterm_user_vars_preexec() {
...
if [[ -z "${WAKTERM_SHELL_SKIP_SEMANTIC_ZONES-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wakterm_semantic_precmd
    blehook PREEXEC+=__wakterm_semantic_preexec
  else
    precmd_functions+=(__wakterm_semantic_precmd)
    preexec_functions+=(__wakterm_semantic_preexec)
  fi
fi

if [[ -z "${WAKTERM_SHELL_SKIP_USER_VARS-}" ]]; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wakterm_user_vars_precmd
    blehook PREEXEC+=__wakterm_user_vars_preexec
  else
    precmd_functions+=(__wakterm_user_vars_precmd)
    preexec_functions+=(__wakterm_user_vars_preexec)
  fi
fi

if [[ -z "${WAKTERM_SHELL_SKIP_CWD-}" ]] ; then
  if [[ -n "${BLE_VERSION-}" ]]; then
    blehook PRECMD+=__wakterm_osc7
  else
    precmd_functions+=(__wakterm_osc7)
  fi
fi
