...
pub const DEFAULT_WINDOW_CLASS: &str = "org.wezfurlong.wakterm";
...
pub struct StartCommand {
    ...
    /// in your wakterm configuration file.
    #[arg(long = "no-auto-connect")]
    pub no_auto_connect: bool,

    /// If enabled, don't try to ask an existing wakterm GUI instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of wakterm so that you can wait for the command
    /// to complete by waiting for this wakterm process to finish.
    ...
    /// The default is "org.wezfurlong.wakterm".
    ...
    /// instance of wakterm, including error, update and ssh
    ...
    /// wakterm will simply attach and will NOT spawn the specified PROG.
    /// If you omit --attach when using --domain, wakterm will attach
    /// AND then spawn PROG.
...
pub struct SshCommand {
    ...
    /// `wakterm ssh` is able to parse some (but not all!) options
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `wakterm ssh user@host -- bash -l` will spawn bash
