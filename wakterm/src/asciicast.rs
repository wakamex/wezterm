...
impl Header {
    fn new(config: &ConfigHandle, size: PtySize, prog: &[&OsStr]) -> Self {
        ...
        env.insert(
            "WAKTERM_VERSION".to_string(),
            config::wakterm_version().to_string(),
        );
        env.insert(
            "WAKTERM_TARGET_TRIPLE".to_string(),
            config::wakterm_target_triple().to_string(),
        );
...
pub struct RecordCommand {
    ...
    /// the default_cwd defined by your wakterm configuration
    #[arg(long)]
