...
fn default_layout_path() -> PathBuf {
    config::CONFIG_DIRS
        .first()
        .cloned()
        .unwrap_or_else(|| config::HOME_DIR.join(".config").join("wakterm"))
        .join("layout.json")
}

#[derive(Debug, Parser, Clone)]
pub struct SaveLayout {
    /// Output file (default: ~/.config/wakterm/layout.json)
...
pub struct RestoreLayout {
    /// Input file (default: ~/.config/wakterm/layout.json)
    #[arg(value_hint = ValueHint::FilePath)]
    file: Option<PathBuf>,
}
...
mod test {
    ...
    fn file_urls_are_saved_as_paths() {
        let path = url_to_path_string(SerdeUrl {
            url: url::Url::parse("file://fedora/code/wakterm").unwrap(),
        });
        assert_eq!(path, "/code/wakterm");
    }
